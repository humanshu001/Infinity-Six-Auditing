...
\paragraph{30-day forward projection.}

Seeding day 0 with the full 181{,}673 i6 dump and re-running a daily
withdraw + partial-recycle loop produces the following 30-day end-states
(detailed day-by-day curves and an interactive view are available in the
companion file \texttt{charts.html}):

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{3.2cm} p{3.5cm} p{3.0cm} p{4.0cm}}
\toprule
\textbf{Scenario} & \textbf{Daily withdrawals} & \textbf{Recycle rate}
                  & \textbf{Day 30 read-out} \\
\midrule
Conservative & $5{,}000$ USDT / day  & $25\%$ of mint
             & spot $\approx 0.9365$, supply $\approx 752$k i6 \\
Moderate     & $15{,}000$ USDT / day & $50\%$ of mint
             & spot $\approx 0.6851$, supply $\approx 1.14$M i6 \\
Aggressive   & $30{,}000$ USDT / day & $75\%$ of mint
             & spot $\approx 0.3277$, supply $\approx 2.22$M i6 \\
\bottomrule
\end{tabular}
\end{center}

\noindent
Under the aggressive scenario the i6 supply grows by $\approx 3.7\times$
its current value within a single month while $595{,}319.57$ USDT of LP
collateral sits unchanged in the pair -- the protocol does not lose USDT,
but every i6 holder is diluted in proportion to the spiral. After applying
the C-1 + C-2 fixes (TWAP, \texttt{minTokensOut}, \texttt{MAX\_SUPPLY},
\texttt{MAX\_TOKENS\_PER\_USD}), the same aggressive 30-day input flattens
the spot curve within $\approx 5$ days and caps supply growth at the
configured ceiling -- shown side-by-side in
\texttt{charts.html}, Section 7.
...
The \texttt{38\_GasAnalysis} suite (run against the BSC fork with local mocks
to isolate MLM logic from PancakeSwap overhead) produces the following raw
measurements:
...
\midrule
\texttt{invest()} depth 1                          &    775{,}108 & 0.55\% & 2.58\% \\
\texttt{invest()} depth 100                        &  1{,}805{,}116 & 1.28\% & 6.01\% \\
\texttt{invest()} depth 200                        &  1{,}914{,}214 & 1.36\% & 6.38\% \\
\texttt{invest()} 200th direct on sponsor          &  2{,}010{,}637 & 1.43\% & 6.70\% \\
\texttt{withdraw()} 100 packages                   &  2{,}085{,}475 & 1.49\% & 6.95\% \\
\bottomrule
\end{tabular}
\end{center}

The slope across depth is well understood:

\begin{itemize}[leftmargin=*]
    \item \textbf{Cold upline levels (1..100)}: $\approx 10{,}404$ gas per
          level (storage slots first touched).
    \item \textbf{Warm upline levels (101..1000)}: $\approx 1{,}090$ gas per
          level (rewrites of already-touched slots).
\end{itemize}

\subsubsection*{Extrapolation to \texttt{maxDownlineDepth = 600}}

Adding the warm-level slope to the measured depth-200 baseline and including
the real on-chain PancakeSwap overhead ($\approx 350{,}000$ gas for the
swap + \texttt{addLiquidity} + burn):

\[
\underbrace{1{,}914{,}214}_{\text{depth-200 base}} \;+\;
\underbrace{(600 - 200)\times 1{,}090}_{\text{warm-level cost}} \;+\;
\underbrace{350{,}000}_{\text{Pancake overhead}}
\;\approx\;
\textbf{2.70\,M gas}.
\]

This is the worst-case real-world \texttt{invest()} cost at depth 600. It
represents:
...
\midrule
\% of BSC 140M block gas limit                       & $\approx 1.93\%$ \\
\% of standard 30M RPC transaction cap               & $\approx 9.00\%$ \\
\textbf{Saving vs.\ current depth-1000 worst case} & $\approx 440{,}000$ gas \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Headroom.} At depth 600 the worst-case \texttt{invest()}
          ...
          for future logic changes without immediately re-entering the gas
          danger zone.
    \item \textbf{Payout fidelity.} Empirical MLM downlines rarely exceed
          a few hundred sponsored levels in any one user's upline. A 600
          ceiling preserves every reachable production payout path while
          discarding only the deep tail.
    \item \textbf{Aggregate savings.} On the protocol level, every deep-tree
          invest saves roughly $4.4 \times 10^{5}$ gas. Across thousands
          of deposits the cumulative gas saved -- and therefore cumulative
          BNB paid by end users -- is material.
\end{itemize}
...
\textbf{Scenario} & \textbf{Gas} & \textbf{\% 140M block} & \textbf{\% 30M RPC} \\
\midrule
\texttt{invest()} at depth 1000, 200 directs, 100 packages (extrapolated)
